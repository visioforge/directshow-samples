...
namespace FFMPEG_Source_Demo
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void AddSource()
        {
            ...
            switch (cbBufferingMode.SelectedIndex)
            {
                case 0:
        ...
        private void GetStreams()
        {
            ...
            {
                streamSelect.Count(out var streamsCount);
                for (int i = 0; i < streamsCount; i++)
                {
                    streamSelect.Info(
                        i, out var mt, out _, out _, out _, out var name, out _, out _);
        ...
        private void cbVideoStream_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (streamSelect != null)
            {
                ...
                {
                    streamSelect.Info(
                        i, out var mt, out _, out _, out var name, out _, out _, out _);
                    if (mt.majorType == MediaType.Video)
                    {
                        if (k == cbVideoStream.SelectedIndex)
                        {
        ...
        private void cbAudioStream_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (streamSelect != null)
            {
                ...
                for (int i = 0; i < streamsCount; i++)
                {
                    ...
                    {
                        if (k == cbAudioStream.SelectedIndex)
                        {
                            streamSelect.Enable(i, AMStreamSelectEnableFlags.Enable);
                        }
                        else
