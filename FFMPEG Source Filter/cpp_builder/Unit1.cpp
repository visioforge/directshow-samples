...
void TForm1::AddSource()
{
    ...
    hr = CoCreateInstance(CLSID_FFmpegSourceFilter, NULL, CLSCTX_INPROC_SERVER,
        IID_IBaseFilter, (void**)&sourceFilter);
    if (FAILED(hr)) {
        // Handle error
    }
    hr = filterGraph->AddFilter(sourceFilter, L"FFMPEG Source");
    if (FAILED(hr)) {
        // Handle error
    }

    // Configure the FFMPEG source filter if it supports custom configuration interface
    IFFmpegSourceSettings* filterConfig = NULL;
    hr = sourceFilter->QueryInterface(
        IID_IFFmpegSourceSettings, (void**)&filterConfig);
    if (SUCCEEDED(hr) && filterConfig) {
        switch (cbBufferingMode->ItemIndex) {
            case 0:
                filterConfig->SetBufferingMode(FFMPEG_SOURCE_BUFFERING_MODE::
        ...
        filterConfig->SetLoadTimeOut(StrToInt(edConnectionTimeOut->Text));
        filterConfig->Release(); // Release the interface after use
    }
...
void TForm1::GetStreams()
{
    cbVideoStream->Items->Clear(); // Assuming this method clears the items
    cbAudioStream->Items->Clear();

    IAMStreamSelect* streamSelect = nullptr;
    sourceFilter->QueryInterface(IID_IAMStreamSelect, (void**)&streamSelect);
    if (streamSelect != nullptr) {
        DWORD streamCount = 0;
        streamSelect->Count(&streamCount);
        for (DWORD i = 0; i < streamCount; i++) {
            AM_MEDIA_TYPE* pmt;
            WCHAR* szName;
            streamSelect->Info(
                i, &pmt, nullptr, nullptr, nullptr, &szName, nullptr, nullptr);
            if (pmt->majortype == MEDIATYPE_Video) {
                cbVideoStream->Items->Add(
                    szName); // Assuming this method adds the item
            } else if (pmt->majortype == MEDIATYPE_Audio) {
                cbAudioStream->Items->Add(szName);
            }
            CoTaskMemFree(szName);
            //DeleteMediaType(pmt);
        }
        streamSelect->Release();
    }

    if (cbVideoStream->Items->Count > 0) {
        cbVideoStream->ItemIndex = 0;
    }

    if (cbAudioStream->Items->Count > 0) {
        cbAudioStream->ItemIndex = 0;
    }
}
...
void __fastcall TForm1::tbSpeedChange(TObject* Sender)
{
    mediaSeeking->SetRate(tbSpeed->Position / 10.0);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::cbVideoStreamChange(TObject* Sender)
{
    IAMStreamSelect* streamSelect = nullptr;
    HRESULT hr = sourceFilter->QueryInterface(
        IID_IAMStreamSelect, (void**)&streamSelect);
    if (SUCCEEDED(hr) && streamSelect != nullptr) {
        DWORD streamCount = 0;
        hr = streamSelect->Count(&streamCount);
        if (SUCCEEDED(hr)) {
            int k = 0;
            for (DWORD i = 0; i < streamCount; ++i) {
                ...
                hr = streamSelect->Info(
                    i, &mt, &flags, &lcid, nullptr, &name, nullptr, nullptr);
                if (SUCCEEDED(hr)) {
                    if (mt->majortype == MEDIATYPE_Video) {
                        if (k == cbVideoStream->ItemIndex) {
                            streamSelect->Enable(
                                i, AMSTREAMSELECTENABLE_ENABLE);
                        } else {
                            streamSelect->Enable(i, 0);
                        }
                        ++k;
                    }
                }
...
void __fastcall TForm1::cbAudioStreamChange(TObject* Sender)
{
    IAMStreamSelect* streamSelect = nullptr;
    HRESULT hr = sourceFilter->QueryInterface(
        IID_IAMStreamSelect, (void**)&streamSelect);
    if (SUCCEEDED(hr) && streamSelect != nullptr) {
        ...
        hr = streamSelect->Count(&streamCount);
        if (SUCCEEDED(hr)) {
            int k = 0; // Counter for audio streams
            for (DWORD i = 0; i < streamCount; ++i) {
                AM_MEDIA_TYPE* mt = nullptr;
                DWORD flags = 0;
                LCID lcid = 0;
                ...
                hr = streamSelect->Info(
                    i, &mt, &flags, &lcid, nullptr, &name, nullptr, nullptr);
                if (SUCCEEDED(hr) && mt->majortype == MEDIATYPE_Audio) {
                    // Perform enabling/disabling based on the selected index in the audio combo box
                    if (k == cbAudioStream->ItemIndex) {
                        streamSelect->Enable(i, AMSTREAMSELECTENABLE_ENABLE);
                    } else {
                        streamSelect->Enable(i, 0);
                    }
                    k++;
                ...
                if (mt) {
                    //DeleteMediaType(mt); // Properly delete the media type
                }
                CoTaskMemFree(name); // Free the memory allocated for the name
            }
        }
        streamSelect
            ->Release(); // Release the IAMStreamSelect interface when done
    }
}
//---------------------------------------------------------------------------
