...
Namespace VLC_Source_Demo
    Partial Public Class Form1
        ...
        Private Sub AddGraph()
            ...
            captureGraph = DirectCast(New CaptureGraphBuilder2(), ICaptureGraphBuilder2)
            mediaControl = DirectCast(filterGraph, IMediaControl)
            mediaSeeking = DirectCast(filterGraph, IMediaSeeking)
            mediaEventEx = DirectCast(filterGraph, IMediaEventEx)
        ...
        Private Function AddSource() As Boolean
            ...
            If sourceFilter Is Nothing Then
                MessageBox.Show(Me, "Unable to create VLC source filter. Be sure that you've selected the correct CPU architecture for the project.")
                Return False
            End If

            ' load file / network stream
            Dim sourceFilterIntf As IFileSourceFilter = TryCast(sourceFilter, IFileSourceFilter)
        ...
        Private Sub AddVideoRenderer()
            ...
            Dim pConfig As IEVRFilterConfig = DirectCast(videoRenderer, IEVRFilterConfig)
            ...
            Dim getService As MediaFoundation.IMFGetService = DirectCast(videoRenderer, MediaFoundation.IMFGetService)
            ...
            Dim videoDisplayControlObj As Object = Nothing
            getService.GetService(
                MFServices.MR_VIDEO_RENDER_SERVICE,
                GetType(IMFVideoDisplayControl).GUID,
                videoDisplayControlObj)
            
            If videoDisplayControlObj IsNot Nothing Then
                Dim dsMFVideoDisplayControl As IMFVideoDisplayControl = TryCast(videoDisplayControlObj, IMFVideoDisplayControl)
            ...
            Try
                ...
                Try
                    ...
                    If mediaSeeking IsNot Nothing Then
                        ...
                        If Path.GetExtension(edFilename.Text)?.ToLowerInvariant() = ".wmv" OrElse
                           Path.GetExtension(edFilename.Text)?.ToLowerInvariant() = ".asf" Then
                            Thread.Sleep(300)
                        End If
                    End If
        ...
        Private Sub tmProgress_Tick(sender As Object, e As EventArgs)
            ...
            Dim duration As TimeSpan = GetDuration()
            Dim position As TimeSpan = GetPosition()
