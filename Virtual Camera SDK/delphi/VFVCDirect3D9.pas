...
type
  ...
  D3DCOLOR = VFVCDXTypes.D3DCOLOR;
{$EXTERNALSYM D3DCOLOR}
  TD3DColor = VFVCDXTypes.TD3DColor;
...
type
  _D3DVECTOR = VFVCDXTypes._D3DVECTOR;
{$EXTERNALSYM _D3DVECTOR}
  D3DVECTOR = VFVCDXTypes.D3DVECTOR;
{$EXTERNALSYM D3DVECTOR}
  TD3DVector = VFVCDXTypes.TD3DVector;
  PD3DVector = VFVCDXTypes.PD3DVector;
...
{$EXTERNALSYM D3DCS_PLANE5}
  D3DCS_ALL = D3DCS_LEFT or D3DCS_RIGHT or D3DCS_TOP or D3DCS_BOTTOM or
    D3DCS_FRONT or D3DCS_BACK or D3DCS_PLANE0 or D3DCS_PLANE1 or D3DCS_PLANE2 or
...
type
  ...
  _D3DPRIMITIVETYPE = (
{$IFNDEF SUPPORTS_EXPL_ENUMS}
    D3DPT_INVALID_0 { = 0 } , D3DPT_POINTLIST { = 1 } , D3DPT_LINELIST { = 2 } ,
    D3DPT_LINESTRIP { = 3 } , D3DPT_TRIANGLELIST { = 4 } ,
...
type
  _D3DTRANSFORMSTATETYPE = (D3DTS_VIEW = 2, D3DTS_PROJECTION = 3,
...
const
  D3DRS_ZENABLE = TD3DRenderStateType(7);
...
{$EXTERNALSYM D3DRS_ZWRITEENABLE}
  D3DRS_ALPHATESTENABLE = TD3DRenderStateType(15);
...
{$EXTERNALSYM D3DRS_DITHERENABLE}
  D3DRS_ALPHABLENDENABLE = TD3DRenderStateType(27);
...
{$EXTERNALSYM D3DRS_FOGTABLEMODE}
  D3DRS_FOGSTART = TD3DRenderStateType(36);
...
{$EXTERNALSYM D3DRS_BLENDOP}
  D3DRS_POSITIONDEGREE = TD3DRenderStateType(172);
  // NPatch position interpolation degree. D3DDEGREE_LINEAR or D3DDEGREE_CUBIC (default)
{$EXTERNALSYM D3DRS_POSITIONDEGREE}
  D3DRS_NORMALDEGREE = TD3DRenderStateType(173);
  // NPatch normal interpolation degree. D3DDEGREE_LINEAR (default) or D3DDEGREE_QUADRATIC
...
{$EXTERNALSYM D3DRS_ENABLEADAPTIVETESSELLATION}
  D3DRS_TWOSIDEDSTENCILMODE = TD3DRenderStateType(185);
  (* BOOL enable/disable 2 sided stenciling *)
{$EXTERNALSYM D3DRS_TWOSIDEDSTENCILMODE}
  D3DRS_CCW_STENCILFAIL = TD3DRenderStateType(186);
  (* D3DSTENCILOP to do if ccw stencil test fails *)
{$EXTERNALSYM D3DRS_CCW_STENCILFAIL}
  D3DRS_CCW_STENCILZFAIL = TD3DRenderStateType(187);
  (* D3DSTENCILOP to do if ccw stencil test passes and Z test fails *)
{$EXTERNALSYM D3DRS_CCW_STENCILZFAIL}
  D3DRS_CCW_STENCILPASS = TD3DRenderStateType(188);
  (* D3DSTENCILOP to do if both ccw stencil and Z tests pass *)
{$EXTERNALSYM D3DRS_CCW_STENCILPASS}
  D3DRS_CCW_STENCILFUNC = TD3DRenderStateType(189);
  (* D3DCMPFUNC fn.  ccw Stencil Test passes if ((ref & mask) stencilfn (stencil & mask)) is true *)
{$EXTERNALSYM D3DRS_CCW_STENCILFUNC}
  D3DRS_COLORWRITEENABLE1 = TD3DRenderStateType(190);
  (* Additional ColorWriteEnables for the devices that support D3DPMISCCAPS_INDEPENDENTWRITEMASKS *)
{$EXTERNALSYM D3DRS_COLORWRITEENABLE1}
  D3DRS_COLORWRITEENABLE2 = TD3DRenderStateType(191);
  (* Additional ColorWriteEnables for the devices that support D3DPMISCCAPS_INDEPENDENTWRITEMASKS *)
{$EXTERNALSYM D3DRS_COLORWRITEENABLE2}
  D3DRS_COLORWRITEENABLE3 = TD3DRenderStateType(192);
  (* Additional ColorWriteEnables for the devices that support D3DPMISCCAPS_INDEPENDENTWRITEMASKS *)
{$EXTERNALSYM D3DRS_COLORWRITEENABLE3}
  D3DRS_BLENDFACTOR = TD3DRenderStateType(193);
  (* D3DCOLOR used for a constant blend factor during alpha blending for devices that support D3DPBLENDCAPS_BLENDFACTOR *)
{$EXTERNALSYM D3DRS_BLENDFACTOR}
  D3DRS_SRGBWRITEENABLE = TD3DRenderStateType(194);
  (* Enable rendertarget writes to be DE-linearized to SRGB (for formats that expose D3DUSAGE_QUERY_SRGBWRITE) *)
{$EXTERNALSYM D3DRS_SRGBWRITEENABLE}
  D3DRS_DEPTHBIAS = TD3DRenderStateType(195);
{$EXTERNALSYM D3DRS_DEPTHBIAS}
  D3DRS_WRAP8 = TD3DRenderStateType(198);
...
{$EXTERNALSYM D3DRS_WRAP15}
  D3DRS_SEPARATEALPHABLENDENABLE = TD3DRenderStateType(206);
  (* TRUE to enable a separate blending function for the alpha channel *)
{$EXTERNALSYM D3DRS_SEPARATEALPHABLENDENABLE}
  D3DRS_SRCBLENDALPHA = TD3DRenderStateType(207);
  (* SRC blend factor for the alpha channel when D3DRS_SEPARATEDESTALPHAENABLE is TRUE *)
{$EXTERNALSYM D3DRS_SRCBLENDALPHA}
  D3DRS_DESTBLENDALPHA = TD3DRenderStateType(208);
  (* DST blend factor for the alpha channel when D3DRS_SEPARATEDESTALPHAENABLE is TRUE *)
{$EXTERNALSYM D3DRS_DESTBLENDALPHA}
  D3DRS_BLENDOPALPHA = TD3DRenderStateType(209);
  (* Blending operation for the alpha channel when D3DRS_SEPARATEDESTALPHAENABLE is TRUE *)
...
type
  _D3DRENDERSTATETYPE = (D3DRS_ZENABLE = 7,
    (* D3DZBUFFERTYPE (or TRUE/FALSE for legacy) *)
    ...
    D3DRS_CLIPPLANEENABLE = 152, D3DRS_POINTSIZE = 154, (* float point size *)
    ...
    D3DRS_MINTESSELLATIONLEVEL = 178, D3DRS_MAXTESSELLATIONLEVEL = 179,
    D3DRS_ADAPTIVETESS_X = 180, D3DRS_ADAPTIVETESS_Y = 181,
    D3DRS_ADAPTIVETESS_Z = 182, D3DRS_ADAPTIVETESS_W = 183,
    D3DRS_ENABLEADAPTIVETESSELLATION = 184, D3DRS_TWOSIDEDSTENCILMODE = 185,
    (* BOOL enable/disable 2 sided stenciling *)
    ...
    (* Enable rendertarget writes to be DE-linearized to SRGB (for formats that expose D3DUSAGE_QUERY_SRGBWRITE) *)
    D3DRS_DEPTHBIAS = 195, D3DRS_WRAP8 = 198,
    (* Additional wrap states for vs_3_0+ attributes with D3DDECLUSAGE_TEXCOORD *)
    D3DRS_WRAP9 = 199, D3DRS_WRAP10 = 200, D3DRS_WRAP11 = 201,
    D3DRS_WRAP12 = 202, D3DRS_WRAP13 = 203, D3DRS_WRAP14 = 204,
    D3DRS_WRAP15 = 205, D3DRS_SEPARATEALPHABLENDENABLE = 206,
    (* TRUE to enable a separate blending function for the alpha channel *)
...
const
  D3DTSS_COLOROP = TD3DTextureStageStateType(1);
...
{$EXTERNALSYM D3DTSS_COLORARG2}
  D3DTSS_ALPHAOP = TD3DTextureStageStateType(4);
...
{$EXTERNALSYM D3DTSS_ALPHAARG2}
  D3DTSS_BUMPENVMAT00 = TD3DTextureStageStateType(7);
  { float (bump mapping matrix) }
{$EXTERNALSYM D3DTSS_BUMPENVMAT00}
  D3DTSS_BUMPENVMAT01 = TD3DTextureStageStateType(8);
  { float (bump mapping matrix) }
{$EXTERNALSYM D3DTSS_BUMPENVMAT01}
  D3DTSS_BUMPENVMAT10 = TD3DTextureStageStateType(9);
  { float (bump mapping matrix) }
{$EXTERNALSYM D3DTSS_BUMPENVMAT10}
  D3DTSS_BUMPENVMAT11 = TD3DTextureStageStateType(10);
  { float (bump mapping matrix) }
{$EXTERNALSYM D3DTSS_BUMPENVMAT11}
  D3DTSS_TEXCOORDINDEX = TD3DTextureStageStateType(11);
  { identifies which set of texture coordinates index this texture }
{$EXTERNALSYM D3DTSS_TEXCOORDINDEX}
  D3DTSS_BUMPENVLSCALE = TD3DTextureStageStateType(22);
  { float scale for bump map luminance }
{$EXTERNALSYM D3DTSS_BUMPENVLSCALE}
  D3DTSS_BUMPENVLOFFSET = TD3DTextureStageStateType(23);
  { float offset for bump map luminance }
{$EXTERNALSYM D3DTSS_BUMPENVLOFFSET}
  D3DTSS_TEXTURETRANSFORMFLAGS = TD3DTextureStageStateType(24);
  { D3DTEXTURETRANSFORMFLAGS controls texture transform }
{$EXTERNALSYM D3DTSS_TEXTURETRANSFORMFLAGS}
  D3DTSS_COLORARG0 = TD3DTextureStageStateType(26);
  { D3DTA_* third arg for triadic ops }
{$EXTERNALSYM D3DTSS_COLORARG0}
  D3DTSS_ALPHAARG0 = TD3DTextureStageStateType(27);
  { D3DTA_* third arg for triadic ops }
{$EXTERNALSYM D3DTSS_ALPHAARG0}
  D3DTSS_RESULTARG = TD3DTextureStageStateType(28);
  { D3DTA_* arg for result (CURRENT or TEMP) }
{$EXTERNALSYM D3DTSS_RESULTARG}
  D3DTSS_CONSTANT = TD3DTextureStageStateType(32);
  { Per-stage constant D3DTA_CONSTANT }
{$EXTERNALSYM D3DTSS_CONSTANT}
  D3DTSS_FORCE_DWORD = TD3DTextureStageStateType($7FFFFFFF);
  { force 32-bit size enum }
{$EXTERNALSYM D3DTSS_FORCE_DWORD}
{$ELSE}

type
  _D3DTEXTURESTAGESTATETYPE = (D3DTSS_COLOROP = 1,
    { D3DTEXTUREOP - per-stage blending controls for color channels }
...
{$IFNDEF SUPPORTS_EXPL_ENUMS}
    D3DSAMP_invalid_0 { = 0 } , D3DSAMP_ADDRESSU { = 1 } ,
    ...
    { When multi-element texture is assigned to sampler, this
...
type
  ...
  //
  _D3DDECLUSAGE = (D3DDECLUSAGE_POSITION, // = 0
...
type
  _D3DDECLMETHOD = (D3DDECLMETHOD_DEFAULT, // = 0,
    D3DDECLMETHOD_PARTIALU, D3DDECLMETHOD_PARTIALV, D3DDECLMETHOD_CROSSUV,
    // Normal
...
{$EXTERNALSYM D3DVERTEXELEMENT9}
  ...
  //
const
  D3DDECL_END: TD3DVertexElement9 = (Stream: $FF; Offset: 0;
    _Type: D3DDECLTYPE_UNUSED; Method: TD3DDeclMethod(0);
    Usage: TD3DDeclUsage(0); UsageIndex: 0);
...
type
  _D3DSHADER_INSTRUCTION_OPCODE_TYPE = {$IFDEF TYPE_IDENTITY} type
  {$ENDIF}DWORD;
...
type
  _D3DSAMPLER_TEXTURE_TYPE = (D3DSTT_UNKNOWN = 0 shl D3DSP_TEXTURETYPE_SHIFT,
    // uninitialized value
    D3DSTT_2D = 2 shl D3DSP_TEXTURETYPE_SHIFT,
    // dcl_2d s# (for declaring a 2-D texture)
    D3DSTT_CUBE = 3 shl D3DSP_TEXTURETYPE_SHIFT,
...
type
  _D3DSHADER_PARAM_REGISTER_TYPE = (D3DSPR_TEMP = 0, // Temporary Register File
    D3DSPR_INPUT = 1, // Input Register File
    D3DSPR_CONST = 2, // Constant Register File
    D3DSPR_ADDR = 3, // Address Register (VS)
    D3DSPR_TEXTURE = 3, // Texture Register File (PS)
...
{$MINENUMSIZE 4} // Restores enums to be 4 byte in size
  ...
  //
  _D3DVS_RASTOUT_OFFSETS = (D3DSRO_POSITION, // = 0,
    D3DSRO_FOG, D3DSRO_POINT_SIZE);
...
type
  // High order surfaces
  //
  _D3DBASISTYPE = (D3DBASIS_BEZIER { = 0 } , D3DBASIS_BSPLINE { = 1 } ,
...
{$EXTERNALSYM D3DBASISTYPE}
  ...
  _D3DDEGREETYPE = (
{$IFNDEF SUPPORTS_EXPL_ENUMS}
    D3DDEGREE_invalid_0 { = 0 } , D3DDEGREE_LINEAR { = 1 } ,
    D3DDEGREE_QUADRATIC { = 2 } , D3DDEGREE_CUBIC { = 3 } ,
    D3DDEGREE_invalid_4 { = 4 } , D3DDEGREE_QUINTIC { = 5 }
{$ELSE}
    D3DDEGREE_LINEAR = 1, D3DDEGREE_QUADRATIC = 2, D3DDEGREE_CUBIC = 3,
    D3DDEGREE_QUINTIC = 5
{$ENDIF}
    );
...
{$EXTERNALSYM D3DDEGREETYPE}
  ...
  _D3DPATCHEDGESTYLE = (D3DPATCHEDGE_DISCRETE { = 0 } ,
...
{$EXTERNALSYM D3DPATCHEDGESTYLE}
  ...
  _D3DSTATEBLOCKTYPE = (
{$IFNDEF SUPPORTS_EXPL_ENUMS}
    D3DSBT_INVALID_0, D3DSBT_ALL { = 1 } , // capture all state
...
type
  ...
  _D3DDEVTYPE = (
{$IFNDEF SUPPORTS_EXPL_ENUMS}
    D3DDEVTYPE_INVALID_0, D3DDEVTYPE_HAL { = 1 } , D3DDEVTYPE_REF { = 2 } ,
    D3DDEVTYPE_SW { = 3 } ,
...
{$IFNDEF SUPPORTS_EXPL_ENUMS}
    ...
    D3DMULTISAMPLE_2_SAMPLES { =  2 } , D3DMULTISAMPLE_3_SAMPLES { =  3 } ,
    D3DMULTISAMPLE_4_SAMPLES { =  4 } , D3DMULTISAMPLE_5_SAMPLES { =  5 } ,
    D3DMULTISAMPLE_6_SAMPLES { =  6 } , D3DMULTISAMPLE_7_SAMPLES { =  7 } ,
    D3DMULTISAMPLE_8_SAMPLES { =  8 } , D3DMULTISAMPLE_9_SAMPLES { =  9 } ,
    D3DMULTISAMPLE_10_SAMPLES { = 10 } , D3DMULTISAMPLE_11_SAMPLES { = 11 } ,
    D3DMULTISAMPLE_12_SAMPLES { = 12 } , D3DMULTISAMPLE_13_SAMPLES { = 13 } ,
    D3DMULTISAMPLE_14_SAMPLES { = 14 } , D3DMULTISAMPLE_15_SAMPLES { = 15 } ,
...
{$EXTERNALSYM D3DFMT_A2W10V10U10}
  ...
  D3DFMT_UYVY = Byte('U') or (Byte('Y') shl 8) or (Byte('V') shl 16) or
    (Byte('Y') shl 24);
{$EXTERNALSYM D3DFMT_UYVY}
  // D3DFMT_R8G8_B8G8            = MAKEFOURCC('R', 'G', 'B', 'G'),
  D3DFMT_R8G8_B8G8 = Byte('R') or (Byte('G') shl 8) or (Byte('B') shl 16) or
    (Byte('G') shl 24);
{$EXTERNALSYM D3DFMT_R8G8_B8G8}
  // D3DFMT_YUY2                 = MAKEFOURCC('Y', 'U', 'Y', '2'),
  D3DFMT_YUY2 = Byte('Y') or (Byte('U') shl 8) or (Byte('Y') shl 16) or
    (Byte('2') shl 24);
{$EXTERNALSYM D3DFMT_YUY2}
  // D3DFMT_G8R8_G8B8            = MAKEFOURCC('G', 'R', 'G', 'B'),
  D3DFMT_G8R8_G8B8 = Byte('G') or (Byte('R') shl 8) or (Byte('G') shl 16) or
    (Byte('B') shl 24);
{$EXTERNALSYM D3DFMT_G8R8_G8B8}
  // D3DFMT_DXT1                 = MAKEFOURCC('D', 'X', 'T', '1'),
  D3DFMT_DXT1 = Byte('D') or (Byte('X') shl 8) or (Byte('T') shl 16) or
    (Byte('1') shl 24);
{$EXTERNALSYM D3DFMT_DXT1}
  // D3DFMT_DXT2                 = MAKEFOURCC('D', 'X', 'T', '2'),
  D3DFMT_DXT2 = Byte('D') or (Byte('X') shl 8) or (Byte('T') shl 16) or
    (Byte('2') shl 24);
{$EXTERNALSYM D3DFMT_DXT2}
  // D3DFMT_DXT3                 = MAKEFOURCC('D', 'X', 'T', '3'),
  D3DFMT_DXT3 = Byte('D') or (Byte('X') shl 8) or (Byte('T') shl 16) or
    (Byte('3') shl 24);
{$EXTERNALSYM D3DFMT_DXT3}
  // D3DFMT_DXT4                 = MAKEFOURCC('D', 'X', 'T', '4'),
  D3DFMT_DXT4 = Byte('D') or (Byte('X') shl 8) or (Byte('T') shl 16) or
    (Byte('4') shl 24);
{$EXTERNALSYM D3DFMT_DXT4}
  // D3DFMT_DXT5                 = MAKEFOURCC('D', 'X', 'T', '5'),
  D3DFMT_DXT5 = Byte('D') or (Byte('X') shl 8) or (Byte('T') shl 16) or
    (Byte('5') shl 24);
...
{$EXTERNALSYM D3DFMT_Q16W16V16U16}
  ...
  D3DFMT_MULTI2_ARGB8 = Byte('M') or (Byte('E') shl 8) or (Byte('T') shl 16) or
    (Byte('1') shl 24);
...
{$ENDIF}
  ...
  D3DFMT_ATI2 = Byte('A') or (Byte('T') shl 8) or (Byte('I') shl 16) or
    (Byte('2') shl 24);
...
type
  _D3DFORMAT = (D3DFMT_UNKNOWN = 0,
    ...
    D3DFMT_A8X8V8U8 = 68, D3DFMT_L8X8V8U8 = 69,

    // D3DFMT_UYVY                 = MAKEFOURCC('U', 'Y', 'V', 'Y'),
    D3DFMT_UYVY = Byte('U') or (Byte('Y') shl 8) or (Byte('V') shl 16) or
    (Byte('Y') shl 24),
    // D3DFMT_RGBG                 = MAKEFOURCC('R', 'G', 'B', 'G'),
    D3DFMT_RGBG = Byte('R') or (Byte('G') shl 8) or (Byte('B') shl 16) or
    (Byte('G') shl 24),
    // D3DFMT_YUY2                 = MAKEFOURCC('Y', 'U', 'Y', '2'),
    D3DFMT_YUY2 = Byte('Y') or (Byte('U') shl 8) or (Byte('Y') shl 16) or
    (Byte('2') shl 24),
    // D3DFMT_GRGB                 = MAKEFOURCC('G', 'R', 'G', 'B'),
    D3DFMT_GRGB = Byte('G') or (Byte('R') shl 8) or (Byte('G') shl 16) or
    (Byte('B') shl 24),
    // D3DFMT_DXT1                 = MAKEFOURCC('D', 'X', 'T', '1'),
    D3DFMT_DXT1 = Byte('D') or (Byte('X') shl 8) or (Byte('T') shl 16) or
    (Byte('1') shl 24),
...
{$ENDIF}
    ...
    // D3DFMT_MULTI2_ARGB8         = MAKEFOURCC('M','E','T','1'),
    D3DFMT_MULTI2_ARGB8 = Byte('M') or (Byte('E') shl 8) or (Byte('T') shl 16)
    or (Byte('1') shl 24),
...
{$EXTERNALSYM D3DDEVICE_CREATION_PARAMETERS}
  ...
  _D3DSWAPEFFECT = (
{$IFNDEF SUPPORTS_EXPL_ENUMS}
    D3DSWAPEFFECT_INVALID_0 { = 0 } , D3DSWAPEFFECT_DISCARD { = 1 } ,
    D3DSWAPEFFECT_FLIP { = 2 } , D3DSWAPEFFECT_COPY { = 3 }
{$ELSE}
...
{$EXTERNALSYM D3DSWAPEFFECT}
  ...
  { Pool types }
  _D3DPOOL = (D3DPOOL_DEFAULT { = 0 } , D3DPOOL_MANAGED { = 1 } ,
    D3DPOOL_SYSTEMMEM { = 2 } , D3DPOOL_SCRATCH { = 3 }
    );
...
{$EXTERNALSYM D3DGAMMARAMP}
  ...
  { Back buffer types }
  _D3DBACKBUFFER_TYPE = (D3DBACKBUFFER_TYPE_MONO { = 0 } ,
    D3DBACKBUFFER_TYPE_LEFT { = 1 } , D3DBACKBUFFER_TYPE_RIGHT { = 2 }
    );
...
{$EXTERNALSYM D3DBACKBUFFER_TYPE}
  ...
  _D3DRESOURCETYPE = (
{$IFNDEF SUPPORTS_EXPL_ENUMS}
    D3DRTYPE_INVALID_0 { =  0 } , D3DRTYPE_SURFACE { =  1 } ,
    D3DRTYPE_VOLUME { =  2 } , D3DRTYPE_TEXTURE { =  3 } ,
    D3DRTYPE_VOLUMETEXTURE { =  4 } , D3DRTYPE_CUBETEXTURE { =  5 } ,
    D3DRTYPE_VERTEXBUFFER { =  6 } , D3DRTYPE_INDEXBUFFER { =  7 }
    // if this changes, change _D3DDEVINFO_RESOURCEMANAGER definition
{$ELSE}
...
type
  ...
  _D3DCUBEMAP_FACES = (D3DCUBEMAP_FACE_POSITIVE_X { = 0 } ,
    D3DCUBEMAP_FACE_NEGATIVE_X { = 1 } , D3DCUBEMAP_FACE_POSITIVE_Y { = 2 } ,
    D3DCUBEMAP_FACE_NEGATIVE_Y { = 3 } , D3DCUBEMAP_FACE_POSITIVE_Z { = 4 } ,
...
{$MINENUMSIZE 1} // Forces TD3DQueryType be 1 byte enum
  _D3DQUERYTYPE = (D3DQUERYTYPE_VCACHE = 4, { D3DISSUE_END }
    D3DQUERYTYPE_RESOURCEMANAGER = 5, { D3DISSUE_END }
    D3DQUERYTYPE_VERTEXSTATS = 6, { D3DISSUE_END }
    D3DQUERYTYPE_EVENT = 8, { D3DISSUE_END }
    D3DQUERYTYPE_OCCLUSION = 9, { D3DISSUE_BEGIN, D3DISSUE_END }
...
const
  ...
  // Tells the runtime to issue the end of a query, changing it's state to "non-signaled".
{$EXTERNALSYM D3DISSUE_END}
  D3DISSUE_BEGIN = (1 shl 1);
  // Tells the runtime to issue the beginng of a query.
{$EXTERNALSYM D3DISSUE_BEGIN}
  // Flags field for GetData
  D3DGETDATA_FLUSH = (1 shl 0);
  // Tells the runtime to flush if the query is outstanding.
...
{$EXTERNALSYM D3DDEVINFO_D3DVERTEXSTATS}
  ...
  _D3DDEVINFO_VCACHE = packed record
    Pattern: DWORD;
    (* bit pattern, return value must be FOUR_CC('C', 'A', 'C', 'H') *)
    OptMethod: DWORD;
...
{$IFDEF DIRECT3D_VERSION_9_VISTA}
  ...
  _D3DCOMPOSERECTSOP = (D3DCOMPOSERECTS_INVALID_0,
    D3DCOMPOSERECTS_COPY { = 1 } , D3DCOMPOSERECTS_OR { = 2 } ,
    D3DCOMPOSERECTS_AND { = 3 } , D3DCOMPOSERECTS_NEG { = 4 }
    );
...
{$EXTERNALSYM D3DCOMPOSERECTDESC}
  ...
  _D3DCOMPOSERECTDESTINATION = record
    ...
    X, y: Smallint;
    // Top-left coordinates of the rect in the destination surface
  end;
...
{$EXTERNALSYM D3DPRESENTSTATS}
  ...
  D3DSCANLINEORDERING = (D3DSCANLINEORDERING_INVALID_0,
...
{$EXTERNALSYM D3DDISPLAYMODEFILTER}
  ...
  D3DDISPLAYROTATION = (
{$IFNDEF SUPPORTS_EXPL_ENUMS}
    D3DDISPLAYROTATION_INVALID_0, D3DDISPLAYROTATION_IDENTITY { = 1 } ,
    // No rotation.
...
type
  ...
  _D3DCAPS9 = record
    ...
    // max value storable in registers of ps.1.x shaders
    ...
    // ordinal of master adaptor for adapter group
    ...
    // D3DPTFILTERCAPS for IDirect3DTexture9's for texture, used in vertex shaders
    MaxVShaderInstructionsExecuted: DWORD;
    // maximum number of vertex shader instructions that can be executed
    MaxPShaderInstructionsExecuted: DWORD;
    // maximum number of pixel shader instructions that can be executed
...
{$EXTERNALSYM D3DCAPS3_LINEAR_TO_SRGB_PRESENTATION}
  D3DCAPS3_COPY_TO_VIDMEM = $00000100;
  { Device can acclerate copies from sysmem to local vidmem }
{$EXTERNALSYM D3DCAPS3_COPY_TO_VIDMEM}
  D3DCAPS3_COPY_TO_SYSTEMMEM = $00000200;
  { Device can acclerate copies from local vidmem to sysmem }
...
{$EXTERNALSYM D3DDEVCAPS_PUREDEVICE}
  D3DDEVCAPS_QUINTICRTPATCHES = $00200000;
...
{$EXTERNALSYM D3DDEVCAPS_RTPATCHES}
  D3DDEVCAPS_RTPATCHHANDLEZERO = $00800000;
...
{$EXTERNALSYM D3DPMISCCAPS_BLENDOP}
  D3DPMISCCAPS_NULLREFERENCE = $00001000;
  { Reference Device that doesnt render }
{$EXTERNALSYM D3DPMISCCAPS_NULLREFERENCE}
  D3DPMISCCAPS_INDEPENDENTWRITEMASKS = $00004000;
  { Device supports independent write masks for MET or MRT }
{$EXTERNALSYM D3DPMISCCAPS_INDEPENDENTWRITEMASKS}
  D3DPMISCCAPS_PERSTAGECONSTANT = $00008000;
  { Device supports per-stage constants }
{$EXTERNALSYM D3DPMISCCAPS_PERSTAGECONSTANT}
  D3DPMISCCAPS_FOGANDSPECULARALPHA = $00010000;
...
{$EXTERNALSYM D3DPMISCCAPS_FOGANDSPECULARALPHA}
  D3DPMISCCAPS_SEPARATEALPHABLEND = $00020000;
  { Device supports separate blend settings for the alpha channel }
{$EXTERNALSYM D3DPMISCCAPS_SEPARATEALPHABLEND}
  D3DPMISCCAPS_MRTINDEPENDENTBITDEPTHS = $00040000;
  { Device supports different bit depths for MRT }
{$EXTERNALSYM D3DPMISCCAPS_MRTINDEPENDENTBITDEPTHS}
  D3DPMISCCAPS_MRTPOSTPIXELSHADERBLENDING = $00080000;
  { Device supports post-pixel shader operations for MRT }
{$EXTERNALSYM D3DPMISCCAPS_MRTPOSTPIXELSHADERBLENDING}
  D3DPMISCCAPS_FOGVERTEXCLAMPED = $00100000;
  { Device clamps fog blend factor per vertex }
{$EXTERNALSYM D3DPMISCCAPS_FOGVERTEXCLAMPED}
{$IFDEF DIRECT3D_VERSION_9_VISTA}
  D3DPMISCCAPS_POSTBLENDSRGBCONVERT = $00200000;
  { Indicates device can perform conversion to sRGB after blending. }
...
{$EXTERNALSYM D3DPRASTERCAPS_ZFOG}
  D3DPRASTERCAPS_COLORPERSPECTIVE = $00400000;
...
{$EXTERNALSYM D3DPBLENDCAPS_BOTHINVSRCALPHA}
  D3DPBLENDCAPS_BLENDFACTOR = $00002000;
  { Supports both D3DBLEND_BLENDFACTOR and D3DBLEND_INVBLENDFACTOR }
...
{$EXTERNALSYM D3DPTEXTURECAPS_MIPMAP}
  D3DPTEXTURECAPS_MIPVOLUMEMAP = $00008000;
...
{$IFDEF DIRECT3D_VERSION_9_VISTA}
  D3DPTFILTERCAPS_CONVOLUTIONMONO = $00040000;
  { Min and Mag for the convolution mono filter }
...
{$EXTERNALSYM IDirect3D9}
  ...
  IDirect3D9 = interface(IUnknown)
    ...
    (* ** IDirect3D9 methods ** *)
    function RegisterSoftwareDevice(pInitializeFunction: Pointer)
      : HResult; stdcall;
    function GetAdapterCount: LongWord; stdcall;
    function GetAdapterIdentifier(Adapter: LongWord; Flags: DWORD;
      out pIdentifier: TD3DAdapterIdentifier9): HResult; stdcall;
    function GetAdapterModeCount(Adapter: LongWord; Format: TD3DFormat)
      : LongWord; stdcall;
    function EnumAdapterModes(Adapter: LongWord; Format: TD3DFormat;
      Mode: LongWord; out pMode: TD3DDisplayMode): HResult; stdcall;
    function GetAdapterDisplayMode(Adapter: LongWord;
      out pMode: TD3DDisplayMode): HResult; stdcall;
    function CheckDeviceType(Adapter: LongWord; CheckType: TD3DDevType;
      AdapterFormat, BackBufferFormat: TD3DFormat; Windowed: Bool)
      : HResult; stdcall;
    ...
    function CheckDeviceMultiSampleType(Adapter: LongWord;
      DeviceType: TD3DDevType; SurfaceFormat: TD3DFormat; Windowed: Bool;
      MultiSampleType: TD3DMultiSampleType; pQualityLevels: PDWORD)
      : HResult; stdcall;
    function CheckDepthStencilMatch(Adapter: LongWord; DeviceType: TD3DDevType;
      AdapterFormat, RenderTargetFormat, DepthStencilFormat: TD3DFormat)
      : HResult; stdcall;
    function CheckDeviceFormatConversion(Adapter: LongWord;
      DeviceType: TD3DDevType; SourceFormat, TargetFormat: TD3DFormat)
      : HResult; stdcall;
    function GetDeviceCaps(Adapter: LongWord; DeviceType: TD3DDevType;
      out pCaps: TD3DCaps9): HResult; stdcall;
...
{$EXTERNALSYM IDirect3D9Ex}
  ...
  IDirect3D9Ex = interface(IDirect3D9)
    ...
    function GetAdapterModeCountEx(Adapter: LongWord;
      const pFilter: PD3DDisplayModeFilter): LongWord; stdcall;
    function EnumAdapterModesEx(Adapter: LongWord;
      const pFilter: PD3DDisplayModeFilter; Mode: LongWord;
      pMode: PD3DDisplayModeEx): HResult; stdcall;
    function GetAdapterDisplayModeEx(Adapter: LongWord;
      pMode: PD3DDisplayModeEx; pRotation: PD3DDisplayRotation)
      : HResult; stdcall;
    function CreateDeviceEx(Adapter: LongWord; DeviceType: TD3DDevType;
      hFocusWindow: HWND; BehaviorFlags: DWORD;
      pPresentationParameters: PD3DPresentParameters;
      pFullscreenDisplayMode: PD3DDisplayModeEx;
      out ppReturnedDeviceInterface: IDirect3DDevice9Ex): HResult; stdcall;
    function GetAdapterLUID(Adapter: LongWord; pLUID: PLargeInteger { *LUID } )
      : HResult; stdcall;
  end;
{$ENDIF}
  { SwapChain }
...
{$EXTERNALSYM IDirect3DDevice9}
  ...
  IDirect3DDevice9 = interface(IUnknown)
    ...
    function GetDisplayMode(iSwapChain: LongWord; out pMode: TD3DDisplayMode)
      : HResult; stdcall;
    function GetCreationParameters(out pParameters
      : TD3DDeviceCreationParameters): HResult; stdcall;
    function SetCursorProperties(XHotSpot, YHotSpot: LongWord;
      pCursorBitmap: IDirect3DSurface9): HResult; stdcall;
    procedure SetCursorPosition(XScreenSpace, YScreenSpace: LongWord;
      Flags: DWORD); stdcall;
    function ShowCursor(bShow: Bool): Bool; stdcall;
    function CreateAdditionalSwapChain(const pPresentationParameters
      : TD3DPresentParameters; out pSwapChain: IDirect3DSwapChain9)
      : HResult; stdcall;
    function GetSwapChain(iSwapChain: LongWord;
      out pSwapChain: IDirect3DSwapChain9): HResult; stdcall;
    function GetNumberOfSwapChains: LongWord; stdcall;
    function Reset(const pPresentationParameters: TD3DPresentParameters)
      : HResult; stdcall;
    function Present(pSourceRect, pDestRect: PRect; hDestWindowOverride: HWND;
      pDirtyRegion: PRgnData): HResult; stdcall;
    function GetBackBuffer(iSwapChain: LongWord; iBackBuffer: LongWord;
      _Type: TD3DBackbufferType; out ppBackBuffer: IDirect3DSurface9)
      : HResult; stdcall;
    function GetRasterStatus(iSwapChain: LongWord;
      out pRasterStatus: TD3DRasterStatus): HResult; stdcall;
    function SetDialogBoxMode(bEnableDialogs: Bool): HResult; stdcall;
    procedure SetGammaRamp(iSwapChain: LongWord; Flags: DWORD;
      const pRamp: TD3DGammaRamp); stdcall;
    procedure GetGammaRamp(iSwapChain: LongWord;
      out pRamp: TD3DGammaRamp); stdcall;
    function CreateTexture(Width, Height, Levels: LongWord; Usage: DWORD;
      Format: TD3DFormat; Pool: TD3DPool; out ppTexture: IDirect3DTexture9;
      pSharedHandle: PHandle): HResult; stdcall;
    function CreateVolumeTexture(Width, Height, Depth, Levels: LongWord;
      Usage: DWORD; Format: TD3DFormat; Pool: TD3DPool;
      out ppVolumeTexture: IDirect3DVolumeTexture9; pSharedHandle: PHandle)
      : HResult; stdcall;
    function CreateCubeTexture(EdgeLength, Levels: LongWord; Usage: DWORD;
      Format: TD3DFormat; Pool: TD3DPool;
      out ppCubeTexture: IDirect3DCubeTexture9; pSharedHandle: PHandle)
      : HResult; stdcall;
    function CreateVertexBuffer(Length: LongWord; Usage, FVF: DWORD;
      Pool: TD3DPool; out ppVertexBuffer: IDirect3DVertexBuffer9;
      pSharedHandle: PHandle): HResult; stdcall;
    function CreateIndexBuffer(Length: LongWord; Usage: DWORD;
      Format: TD3DFormat; Pool: TD3DPool;
      out ppIndexBuffer: IDirect3DIndexBuffer9; pSharedHandle: PHandle)
      : HResult; stdcall;
    function CreateRenderTarget(Width, Height: LongWord; Format: TD3DFormat;
      MultiSample: TD3DMultiSampleType; MultiSampleQuality: DWORD;
      Lockable: Bool; out ppSurface: IDirect3DSurface9; pSharedHandle: PHandle)
      : HResult; stdcall;
    function CreateDepthStencilSurface(Width, Height: LongWord;
      Format: TD3DFormat; MultiSample: TD3DMultiSampleType;
      MultiSampleQuality: DWORD; Discard: Bool;
      out ppSurface: IDirect3DSurface9; pSharedHandle: PHandle)
      : HResult; stdcall;
    function UpdateSurface(pSourceSurface: IDirect3DSurface9;
      pSourceRect: PRect; pDestinationSurface: IDirect3DSurface9;
      pDestPoint: PPoint): HResult; stdcall;
    function UpdateTexture(pSourceTexture, pDestinationTexture
      : IDirect3DBaseTexture9): HResult; stdcall;
    function GetRenderTargetData(pRenderTarget, pDestSurface: IDirect3DSurface9)
      : HResult; stdcall;
    function GetFrontBufferData(iSwapChain: LongWord;
      pDestSurface: IDirect3DSurface9): HResult; stdcall;
    function StretchRect(pSourceSurface: IDirect3DSurface9; pSourceRect: PRect;
      pDestSurface: IDirect3DSurface9; pDestRect: PRect;
      Filter: TD3DTextureFilterType): HResult; stdcall;
    function ColorFill(pSurface: IDirect3DSurface9; PRect: PRect;
      color: TD3DColor): HResult; stdcall;
    function CreateOffscreenPlainSurface(Width, Height: LongWord;
      Format: TD3DFormat; Pool: TD3DPool; out ppSurface: IDirect3DSurface9;
      pSharedHandle: PHandle): HResult; stdcall;
    ...
    function SetDepthStencilSurface(pNewZStencil: IDirect3DSurface9)
      : HResult; stdcall;
    function GetDepthStencilSurface(out ppZStencilSurface: IDirect3DSurface9)
      : HResult; stdcall;
    function BeginScene: HResult; stdcall;
    function EndScene: HResult; stdcall;
    function Clear(Count: DWORD; pRects: PD3DRect; Flags: DWORD;
      color: TD3DColor; Z: Single; Stencil: DWORD): HResult; stdcall;
    function SetTransform(State: TD3DTransformStateType;
      const pMatrix: TD3DMatrix): HResult; stdcall;
    function GetTransform(State: TD3DTransformStateType;
      out pMatrix: TD3DMatrix): HResult; stdcall;
    function MultiplyTransform(State: TD3DTransformStateType;
      const pMatrix: TD3DMatrix): HResult; stdcall;
    ...
    function SetRenderState(State: TD3DRenderStateType; Value: DWORD)
      : HResult; stdcall;
    function GetRenderState(State: TD3DRenderStateType; out pValue: DWORD)
      : HResult; stdcall;
    function CreateStateBlock(_Type: TD3DStateBlockType;
    ...
    function SetClipStatus(const pClipStatus: TD3DClipStatus9)
      : HResult; stdcall;
    function GetClipStatus(out pClipStatus: TD3DClipStatus9): HResult; stdcall;
    function GetTexture(Stage: DWORD; out ppTexture: IDirect3DBaseTexture9)
      : HResult; stdcall;
    function SetTexture(Stage: DWORD; pTexture: IDirect3DBaseTexture9)
      : HResult; stdcall;
    function GetTextureStageState(Stage: DWORD;
      _Type: TD3DTextureStageStateType; out pValue: DWORD): HResult; stdcall;
    function SetTextureStageState(Stage: DWORD;
      _Type: TD3DTextureStageStateType; Value: DWORD): HResult; stdcall;
    function GetSamplerState(Sampler: DWORD; _Type: TD3DSamplerStateType;
      out pValue: DWORD): HResult; stdcall;
    function SetSamplerState(Sampler: DWORD; _Type: TD3DSamplerStateType;
      Value: DWORD): HResult; stdcall;
    function ValidateDevice(out pNumPasses: DWORD): HResult; stdcall;
    function SetPaletteEntries(PaletteNumber: LongWord; pEntries: pPaletteEntry)
      : HResult; stdcall;
    function GetPaletteEntries(PaletteNumber: LongWord; pEntries: pPaletteEntry)
      : HResult; stdcall;
    function SetCurrentTexturePalette(PaletteNumber: LongWord)
      : HResult; stdcall;
    function GetCurrentTexturePalette(out PaletteNumber: LongWord)
      : HResult; stdcall;
    ...
    function DrawPrimitive(PrimitiveType: TD3DPrimitiveType;
      StartVertex, PrimitiveCount: LongWord): HResult; stdcall;
    function DrawIndexedPrimitive(_Type: TD3DPrimitiveType;
      BaseVertexIndex: integer; MinVertexIndex, NumVertices, startIndex,
      primCount: LongWord): HResult; stdcall;
    function DrawPrimitiveUP(PrimitiveType: TD3DPrimitiveType;
      PrimitiveCount: LongWord; const pVertexStreamZeroData;
      VertexStreamZeroStride: LongWord): HResult; stdcall;
    function DrawIndexedPrimitiveUP(PrimitiveType: TD3DPrimitiveType;
      MinVertexIndex, NumVertice, PrimitiveCount: LongWord; const pIndexData;
      IndexDataFormat: TD3DFormat; const pVertexStreamZeroData;
      VertexStreamZeroStride: LongWord): HResult; stdcall;
    function ProcessVertices(SrcStartIndex, DestIndex, VertexCount: LongWord;
      pDestBuffer: IDirect3DVertexBuffer9;
      pVertexDecl: IDirect3DVertexDeclaration9; Flags: DWORD): HResult; stdcall;
    ...
    function SetVertexDeclaration(pDecl: IDirect3DVertexDeclaration9)
      : HResult; stdcall;
    function GetVertexDeclaration(out ppDecl: IDirect3DVertexDeclaration9)
      : HResult; stdcall;
    ...
    function GetVertexShader(out ppShader: IDirect3DVertexShader9)
      : HResult; stdcall;
    function SetVertexShaderConstantF(StartRegister: LongWord;
      pConstantData: PSingle; Vector4fCount: LongWord): HResult; stdcall;
    function GetVertexShaderConstantF(StartRegister: LongWord;
      pConstantData: PSingle; Vector4fCount: LongWord): HResult; stdcall;
    function SetVertexShaderConstantI(StartRegister: LongWord;
      pConstantData: PInteger; Vector4iCount: LongWord): HResult; stdcall;
    function GetVertexShaderConstantI(StartRegister: LongWord;
      pConstantData: PInteger; Vector4iCount: LongWord): HResult; stdcall;
    function SetVertexShaderConstantB(StartRegister: LongWord;
      pConstantData: PBOOL; BoolCount: LongWord): HResult; stdcall;
    function GetVertexShaderConstantB(StartRegister: LongWord;
      pConstantData: PBOOL; BoolCount: LongWord): HResult; stdcall;
    function SetStreamSource(StreamNumber: LongWord;
      pStreamData: IDirect3DVertexBuffer9; OffsetInBytes, Stride: LongWord)
      : HResult; stdcall;
    function GetStreamSource(StreamNumber: LongWord;
      out ppStreamData: IDirect3DVertexBuffer9;
      out pOffsetInBytes, pStride: LongWord): HResult; stdcall;
    function SetStreamSourceFreq(StreamNumber: LongWord; Setting: LongWord)
      : HResult; stdcall;
    function GetStreamSourceFreq(StreamNumber: LongWord; out Setting: LongWord)
      : HResult; stdcall;
    function SetIndices(pIndexData: IDirect3DIndexBuffer9): HResult; stdcall;
    function GetIndices(out ppIndexData: IDirect3DIndexBuffer9)
      : HResult; stdcall;
    ...
    function GetPixelShader(out ppShader: IDirect3DPixelShader9)
      : HResult; stdcall;
    function SetPixelShaderConstantF(StartRegister: LongWord;
      pConstantData: PSingle; Vector4fCount: LongWord): HResult; stdcall;
    function GetPixelShaderConstantF(StartRegister: LongWord;
      pConstantData: PSingle; Vector4fCount: LongWord): HResult; stdcall;
    function SetPixelShaderConstantI(StartRegister: LongWord;
      pConstantData: PInteger; Vector4iCount: LongWord): HResult; stdcall;
    function GetPixelShaderConstantI(StartRegister: LongWord;
      pConstantData: PInteger; Vector4iCount: LongWord): HResult; stdcall;
    function SetPixelShaderConstantB(StartRegister: LongWord;
      pConstantData: PBOOL; BoolCount: LongWord): HResult; stdcall;
    function GetPixelShaderConstantB(StartRegister: LongWord;
      pConstantData: PBOOL; BoolCount: LongWord): HResult; stdcall;
    function DrawRectPatch(Handle: LongWord; pNumSegs: PSingle;
      pTriPatchInfo: PD3DRectPatchInfo): HResult; stdcall;
    function DrawTriPatch(Handle: LongWord; pNumSegs: PSingle;
      pTriPatchInfo: PD3DTriPatchInfo): HResult; stdcall;
    function DeletePatch(Handle: LongWord): HResult; stdcall;
    function CreateQuery(_Type: TD3DQueryType; out ppQuery: IDirect3DQuery9)
      : HResult; stdcall;
  end;
...
{$EXTERNALSYM IDirect3DDevice9Ex}
  ...
  IDirect3DDevice9Ex = interface(IDirect3DDevice9)
    ...
    function SetConvolutionMonoKernel(Width, Height: LongWord;
      rows, columns: PSingle): HResult; stdcall;
    function ComposeRects(pSrc, pDst: IDirect3DSurface9;
      pSrcRectDescs: IDirect3DVertexBuffer9; NumRects: LongWord;
      pDstRectDescs: IDirect3DVertexBuffer9; Operation: TD3DComposeRectsOp;
      Xoffset, Yoffset: integer): HResult; stdcall;
    function PresentEx(const pSourceRect, pDestRect: PRect;
      hDestWindowOverride: HWND; const pDirtyRegion: PRgnData; dwFlags: DWORD)
      : HResult; stdcall;
    ...
    function CheckResourceResidency(pResourceArray: PIDirect3DResource9;
      NumResources: LongWord): HResult; stdcall;
    function SetMaximumFrameLatency(MaxLatency: LongWord): HResult; stdcall;
    function GetMaximumFrameLatency(out pMaxLatency: LongWord)
      : HResult; stdcall;
    function CheckDeviceState(hDestinationWindow: HWND): HResult; stdcall;
    function CreateRenderTargetEx(Width, Height: LongWord; Format: TD3DFormat;
      MultiSample: TD3DMultiSampleType; MultiSampleQuality: DWORD;
      Lockable: Bool; out ppSurface: IDirect3DSurface9; pSharedHandle: PHandle;
      Usage: DWORD): HResult; stdcall;
    function CreateOffscreenPlainSurfaceEx(Width, Height: LongWord;
      Format: TD3DFormat; Pool: TD3DPool; out ppSurface: IDirect3DSurface9;
      pSharedHandle: PHandle; Usage: DWORD): HResult; stdcall;
    function CreateDepthStencilSurfaceEx(Width, Height: LongWord;
      Format: TD3DFormat; MultiSample: TD3DMultiSampleType;
      MultiSampleQuality: DWORD; Discard: Bool;
      out ppSurface: IDirect3DSurface9; pSharedHandle: PHandle; Usage: DWORD)
      : HResult; stdcall;
    function ResetEx(const pPresentationParameters: TD3DPresentParameters;
      const pFullscreenDisplayMode: TD3DDisplayModeEx): HResult; stdcall;
    function GetDisplayModeEx(iSwapChain: LongWord; pMode: PD3DDisplayModeEx;
      pRotation: PD3DDisplayRotation): HResult; stdcall;
  end;
{$ENDIF}
...
{$EXTERNALSYM IDirect3DSwapChain9}
  ...
  IDirect3DSwapChain9 = interface(IUnknown)
    ...
    function Present(pSourceRect, pDestRect: PRect; hDestWindowOverride: HWND;
      pDirtyRegion: PRgnData; dwFlags: DWORD): HResult; stdcall;
    function GetFrontBufferData(pDestSurface: IDirect3DSurface9)
      : HResult; stdcall;
    function GetBackBuffer(iBackBuffer: LongWord; _Type: TD3DBackbufferType;
      out ppBackBuffer: IDirect3DSurface9): HResult; stdcall;
    function GetRasterStatus(out pRasterStatus: TD3DRasterStatus)
      : HResult; stdcall;
    function GetDisplayMode(out pMode: TD3DDisplayMode): HResult; stdcall;
    function GetDevice(out ppDevice: IDirect3DDevice9): HResult; stdcall;
    function GetPresentParameters(out pPresentationParameters
...
{$EXTERNALSYM IDirect3DSwapChain9Ex}
  ...
  IDirect3DSwapChain9Ex = interface(IDirect3DSwapChain9)
    ...
    function GetLastPresentCount(out pLastPresentCount: LongWord)
      : HResult; stdcall;
    function GetPresentStats(out pPresentationStatistics: TD3DPresentStats)
      : HResult; stdcall;
    function GetDisplayModeEx(pMode: PD3DDisplayModeEx;
      pRotation: PD3DDisplayRotation): HResult; stdcall;
  end;
{$ENDIF}
...
{$EXTERNALSYM IDirect3DResource9}
  ...
  IDirect3DResource9 = interface(IUnknown)
    ...
    function GetDevice(out ppDevice: IDirect3DDevice9): HResult; stdcall;
    function SetPrivateData(const refguid: TGUID; const pData: Pointer;
      SizeOfData, Flags: DWORD): HResult; stdcall;
    function GetPrivateData(const refguid: TGUID; pData: Pointer;
...
{$EXTERNALSYM IDirect3DVertexDeclaration9}
  ...
  IDirect3DVertexDeclaration9 = interface(IUnknown)
    ...
    function GetDeclaration(pElement: PD3DVertexElement9;
      out pNumElements: LongWord): HResult; stdcall;
  end;
...
{$EXTERNALSYM IDirect3DVertexShader9}
  ...
  IDirect3DVertexShader9 = interface(IUnknown)
    ...
    function GetDevice(out ppDevice: IDirect3DDevice9): HResult; stdcall;
    function GetFunction(pData: Pointer; out pSizeOfData: LongWord)
      : HResult; stdcall;
  end;
...
{$EXTERNALSYM IDirect3DBaseTexture9}
  ...
  IDirect3DBaseTexture9 = interface(IDirect3DResource9)
    ...
    function SetAutoGenFilterType(FilterType: TD3DTextureFilterType)
      : HResult; stdcall;
...
{$EXTERNALSYM IDirect3DTexture9}
  ...
  IDirect3DTexture9 = interface(IDirect3DBaseTexture9)
    ...
    function GetLevelDesc(Level: LongWord; out pDesc: TD3DSurfaceDesc)
      : HResult; stdcall;
    function GetSurfaceLevel(Level: LongWord;
      out ppSurfaceLevel: IDirect3DSurface9): HResult; stdcall;
    function LockRect(Level: LongWord; out pLockedRect: TD3DLockedRect;
...
{$EXTERNALSYM IDirect3DVolumeTexture9}
  ...
  IDirect3DVolumeTexture9 = interface(IDirect3DBaseTexture9)
    ...
    function GetLevelDesc(Level: LongWord; out pDesc: TD3DVolumeDesc)
      : HResult; stdcall;
    function GetVolumeLevel(Level: LongWord;
      out ppVolumeLevel: IDirect3DVolume9): HResult; stdcall;
    function LockBox(Level: LongWord; out pLockedVolume: TD3DLockedBox;
      pBox: PD3DBox; Flags: DWORD): HResult; stdcall;
...
{$EXTERNALSYM IDirect3DCubeTexture9}
  ...
  IDirect3DCubeTexture9 = interface(IDirect3DBaseTexture9)
    ...
    function GetLevelDesc(Level: LongWord; out pDesc: TD3DSurfaceDesc)
      : HResult; stdcall;
    function GetCubeMapSurface(FaceType: TD3DCubemapFaces; Level: LongWord;
      out ppCubeMapSurface: IDirect3DSurface9): HResult; stdcall;
    function LockRect(FaceType: TD3DCubemapFaces; Level: LongWord;
      out pLockedRect: TD3DLockedRect; PRect: PRect; Flags: DWORD)
      : HResult; stdcall;
    function UnlockRect(FaceType: TD3DCubemapFaces; Level: LongWord)
      : HResult; stdcall;
    function AddDirtyRect(FaceType: TD3DCubemapFaces; pDirtyRect: PRect)
      : HResult; stdcall;
  end;
...
{$EXTERNALSYM IDirect3DVertexBuffer9}
  ...
  IDirect3DVertexBuffer9 = interface(IDirect3DResource9)
    ...
    function Lock(OffsetToLock, SizeToLock: LongWord; out ppbData: Pointer;
...
{$EXTERNALSYM IDirect3DIndexBuffer9}
  ...
  IDirect3DIndexBuffer9 = interface(IDirect3DResource9)
    ...
    function Lock(OffsetToLock, SizeToLock: DWORD; out ppbData: Pointer;
      Flags: DWORD): HResult; stdcall;
    function Unlock: HResult; stdcall;
...
{$EXTERNALSYM IDirect3DSurface9}
  ...
  IDirect3DSurface9 = interface(IDirect3DResource9)
    ...
    function GetContainer(const riid: TGUID; out ppContainer { : Pointer } )
      : HResult; stdcall;
    function GetDesc(out pDesc: TD3DSurfaceDesc): HResult; stdcall;
    function LockRect(out pLockedRect: TD3DLockedRect; PRect: PRect;
      Flags: DWORD): HResult; stdcall;
...
{$EXTERNALSYM IDirect3DVolume9}
  ...
  IDirect3DVolume9 = interface(IUnknown)
    ...
    function GetDevice(out ppDevice: IDirect3DDevice9): HResult; stdcall;
    function SetPrivateData(const refguid: TGUID; const pData;
      SizeOfData, Flags: DWORD): HResult; stdcall;
    function GetPrivateData(const refguid: TGUID; pData: Pointer;
      out pSizeOfData: DWORD): HResult; stdcall;
    function FreePrivateData(const refguid: TGUID): HResult; stdcall;
    function GetContainer(const riid: TGUID; var ppContainer: Pointer)
      : HResult; stdcall;
    function GetDesc(out pDesc: TD3DVolumeDesc): HResult; stdcall;
    function LockBox(out pLockedVolume: TD3DLockedBox; pBox: PD3DBox;
      Flags: DWORD): HResult; stdcall;
...
{$EXTERNALSYM IDirect3DQuery9}
  ...
  IDirect3DQuery9 = interface(IUnknown)
    ...
    function GetData(pData: Pointer; dwSize: DWORD; dwGetDataFlags: DWORD)
      : HResult; stdcall;
  end;
...
function MAKE_D3DHRESULT(Code: DWORD): DWORD; {$IFDEF SUPPORTS_INLINE} inline;
{$ENDIF}
{$EXTERNALSYM MAKE_D3DHRESULT}
// #define MAKE_D3DSTATUS( code )  MAKE_HRESULT( 0, _FACD3D, code )
function MAKE_D3DSTATUS(Code: DWORD): DWORD; {$IFDEF SUPPORTS_INLINE} inline;
{$ENDIF}
...
{$IFDEF DIRECT3D_VERSION_9_VISTA}
  TDirect3DCreate9Ex = function(SDKVersion: LongWord; out d3d9ex: IDirect3D9Ex)
    : HResult; stdcall;
{$ENDIF}
...
{$IFDEF DIRECT3D_VERSION_9_VISTA}
function Direct3DCreate9Ex(SDKVersion: LongWord; out d3d9ex: IDirect3D9Ex)
  : HResult; stdcall;
...
function D3DPERF_BeginEvent(col: TD3DColor; wszName: PWideChar): integer;
  stdcall; external Direct3D9dll;
...
procedure D3DPERF_SetMarker(col: TD3DColor; wszName: PWideChar); stdcall;
  external Direct3D9dll;
{$EXTERNALSYM D3DPERF_SetMarker}
procedure D3DPERF_SetRegion(col: TD3DColor; wszName: PWideChar); stdcall;
  external Direct3D9dll;
...
function D3DSHADER_COMMENT(_DWordSize: DWORD): DWORD;
begin
  Result := ((_DWordSize shl D3DSI_COMMENTSIZE_SHIFT) and
    D3DSI_COMMENTSIZE_MASK) or D3DSIO_COMMENT;
end;
...
function MAKEFOURCC(ch0, ch1, ch2, ch3: Char): DWORD;
begin
  Result := Byte(ch0) or (Byte(ch1) shl 8) or (Byte(ch2) shl 16) or
    (Byte(ch3) shl 24);
end;
...
function _Direct3DCreate9(SDKVersion: LongWord): Pointer;
  external Direct3D9dll name 'Direct3DCreate9';
{$IFDEF DIRECT3D_VERSION_9_VISTA}
function Direct3DCreate9Ex(SDKVersion: LongWord; out d3d9ex: IDirect3D9Ex)
  : HResult; stdcall; external Direct3D9dll;
{$ENDIF}
{$ENDIF}
...
{$ENDIF}
  ...
  if Assigned(Result) then
    Result._Release; // Delphi autoincrement reference count
end;
