...
namespace VisioForge.DirectShowLib.BDA
{
    ...
    Guid("AE1A2884-540E-4077-B1AB-67A34A72298C"),
    InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
    [CLSCompliant(false)]
    public interface IMpeg2PsiParser
    {
        [PreserveSig]
        int GetTransportStreamId([Out] out short pStreamId);

        [PreserveSig]
        int GetPatVersionNumber([Out] out byte pPatVersion);

        [PreserveSig]
        int GetCountOfPrograms([Out] out int pNumOfPrograms);

        [PreserveSig]
        int GetRecordProgramNumber([In] uint dwIndex, [Out] out short pwVal);

        [PreserveSig]
        int GetRecordProgramMapPid([In] uint dwIndex, [Out] out short pwVal);

        [PreserveSig]
        int FindRecordProgramMapPid([In] short wProgramNumber, [Out] out short pwVal);

        [PreserveSig]
        int GetPmtVersionNumber([In] short wProgramNumber, [Out] out byte pPmtVersion);

        [PreserveSig]
        int GetCountOfElementaryStreams([In] short wProgramNumber, [Out] out short pwVal);

        [PreserveSig]
        int GetRecordStreamType([In] short wProgramNumber, [In] uint dwRecordIndex, [Out] out byte pbVal);

        [PreserveSig]
        int GetRecordElementaryPid([In] short wProgramNumber, [In] uint dwRecordIndex, [Out] out short pwVal);
    }
