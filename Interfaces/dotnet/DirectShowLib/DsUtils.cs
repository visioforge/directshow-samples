...
namespace VisioForge.DirectShowLib
{
    ...
    using VisioForge.DirectShowLib.Dvd;
    ...
    /// This class is necessary to enable null paramters passing.
    /// </remarks>
    [Localizable(false), StructLayout(LayoutKind.Explicit)]
    public class DsGuid
    {
        [FieldOffset(0)]
        private Guid guid;
    ...
    public struct NormalizedRect
    {
        ...
        {
            return (Math.Abs(r1.left - r2.left) < 0.01) && (Math.Abs(r1.top - r2.top) < 0.01) && (Math.Abs(r1.right - r2.right) < 0.01) && (Math.Abs(r1.bottom - r2.bottom) < 0.01);
        }
    ...
    public static class DsUtils
    {
        ...
        /// </summary>
        public static void FreePinInfo(ref PinInfo pinInfo)
        {
    ...
    public sealed class DsDevice : IDisposable
    {
        ...
        public static DsDevice[] GetDevicesOfCat(Guid filterCategory, bool allowWaveOut = true, bool doNotIgnore = false)
        {
            ...
            try
            {
                    ...
                    try
                    {
                        try
                        {
                            ...
                            while (enumMon.Next(1, mon, IntPtr.Zero) == 0)
                            {
                                try
                                {
                                    // The devs array now owns this object.  Don't release it if we are going to be successfully
                ...
                if (devret != null)
                {
                    ...
                    foreach (DsDevice device in devret)
                    {
                        device.FillProperties(filterCategory, allowWaveOut, doNotIgnore);

                        if (device.Name == "<ignore>" || string.IsNullOrEmpty(device.Name))
                        {
                            continue;
                        }

                        // Fill duplicates
                        if (filterCategory == FilterCategory.VideoInputDevice)
                        {
                            string s;

                            if (device.Name.IndexOf("Microsoft DV Camera and VCR", StringComparison.Ordinal) != -1)
                            {
                                s = device.Name.Replace("Microsoft DV Camera and VCR", "(DV)");
                                device.Name = device.Description + ' ' + s;
                            }

                            if (device.Name.IndexOf("Microsoft AV/C Tape Subunit Device", StringComparison.Ordinal) != -1)
                            {
                                s = device.Name.Replace("Microsoft AV/C Tape Subunit Device", "(DV/MPEG/Tape Device)");
                                device.Name = device.Description + ' ' + s;
                            }

                            if (dupNames.IndexOf(device.Name) == -1)
                            {
                                dupNames.Add(device.Name);
                            }
                            else
                            {
                                for (int k = 2; k <= 10; k++)
                                {
                                    if (dupNames.IndexOf(device.Name + " (" + k + ')') == -1)
                                    {
                                        device.Name = device.Name + " (" + k + ')';
                                        break;
                                    }
                                }
                        ...
                        else if (filterCategory == FilterCategory.AudioRendererCategory)
                        {
                            if (dupNames.IndexOf(device.Name) == -1)
                            {
                                dupNames.Add(device.Name);
        ...
        /// </summary>
        /// <returns>String or null on error</returns>
        public void FillProperties(Guid filterCategory, bool allowWaveOut, bool doNotIgnore)
        {
            IPropertyBag bag = null;
            string ret = null;
            object bagObj = null;
            ...
            try
            {
                ...
                if (!doNotIgnore)
                {
                    if (filterCategory == FilterCategory.AudioRendererCategory && ret != null)
                    {
                        // sName = name.ToString().Trim();
                        if (ret.Contains("WaveOut") && allowWaveOut)
                        {
                            ret = ret.Replace("WaveOut:", string.Empty).Trim();
                        }
                        else if (ret.Contains("DirectSound"))
                        {
                            ret = ret.Replace("DirectSound:", string.Empty).Trim();
                        }
                        else
                        {
                            ret = "<ignore>";
                ...
                //FourCC
                hr = bag.Read("FccHandler", out val, null);
                //DsError.ThrowExceptionForHR(hr);
                ...
                //Description
                hr = bag.Read("Description", out val, null);
                //DsError.ThrowExceptionForHR(hr);
                ...
                if (filterCategory == FilterCategory.VideoInputDevice)
                {
                    // Name correction
                    //string s;
                    //if (FriendlyName.IndexOf("Microsoft DV Camera and VCR") != -1)
                    //{
                    //    s = FriendlyName.Replace("Microsoft DV Camera and VCR", "(DV)");
                    //    FriendlyName = Description + ' ' + s;
                    //}

                    //if (FriendlyName.IndexOf("Microsoft AV/C Tape Subunit Device") != -1)
                    //{
                    //    s = FriendlyName.Replace("Microsoft AV/C Tape Subunit Device", "(DV/MPEG/Tape Device)");
                    //    FriendlyName = Description + ' ' + s;
                    //}

                    //if (names.IndexOf(FriendlyName) != -1)
                    //{
                    //    int k;
                    //    for (k = 2; k <= 10; k++)
                    //    {
                    //        if (names.IndexOf(FriendlyName + " (" + k + ')') == -1)
                    //        {
                    //            FriendlyName = FriendlyName + " (" + k + ')';
                    //            break;
                    //        }
                    //    }
                    //}
        ...
        /// </summary>
        /// <returns>String or null on error</returns>
        public Guid GetCLSID(Guid filterCategory)
        {
            IPropertyBag bag = null;
            string ret = null;
            object bagObj = null;
        ...
        public static IPin ByName(IBaseFilter vSource, string vPinName)
        {
            ...
            try
            {
                ...
                while (ppEnum.Next(1, pPins, IntPtr.Zero) == 0)
                {
                    ...
                    {
                        DsUtils.FreePinInfo(ref ppinfo);
                        pRet = pPins[0];
                        break;
                    }
