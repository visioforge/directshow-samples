...
namespace VisioForge.DirectShowLib.DirectX
{
    ...
    public interface IDirect3DDevice9
    {
        ...
        int ColorFill();
        int CreateOffscreenPlainSurface(UInt32 Width, UInt32 Height, D3DFORMAT Format, D3DPOOL Pool, [MarshalAs(UnmanagedType.Interface)] out object ppSurface,
            object pSharedHandle);
        int SetRenderTarget();
        [PreserveSig, SuppressUnmanagedCodeSecurity]
    ...
    public interface IDirect3DDevice9Ex : IDirect3DDevice9
    {
        ...
        new int ColorFill();
        new int CreateOffscreenPlainSurface(UInt32 Width, UInt32 Height, D3DFORMAT Format, D3DPOOL Pool, [MarshalAs(UnmanagedType.Interface)] out object ppSurface,
            object pSharedHandle);
    ...
    public interface IDirect3DSurface9
    {
        ...
        int LockRect([Out]out D3DLOCKED_RECT pLockedRect, [In]object pRect, [In]int Flags);
        //int LockRect([In]D3DLOCKED_RECT pLockedRect, [In]object pRect, [In]int Flags);
        void UnlockRect();
        [PreserveSig, SuppressUnmanagedCodeSecurity]
    ...
    public struct D3DCAPS9
    {
        ...
        public UInt32 MasterAdapterOrdinal;       // ordinal of master adaptor for adapter group
        public UInt32 AdapterOrdinalInGroup;      // ordinal inside the adapter group
        public UInt32 NumberOfAdaptersInGroup;    // number of adapters in this adapter group (only if master)
    ...
    public interface IDirect3D9Ex : IDirect3D9
    {
        ...
        [PreserveSig, SuppressUnmanagedCodeSecurity]
        new UInt32 GetAdapterModeCount(uint Adapter, D3DFORMAT Format);

        [PreserveSig, SuppressUnmanagedCodeSecurity]
        new int EnumAdapterModes(uint Adapter, D3DFORMAT Format, uint Mode, [Out] out D3DDISPLAYMODE pMode);

        [PreserveSig, SuppressUnmanagedCodeSecurity]
        new int GetAdapterDisplayMode(UInt16 Adapter, [Out]out D3DDISPLAYMODE pMode);
        ...
        new int CreateDevice(int Adapter,
                         ...
                         [MarshalAs(UnmanagedType.Interface)] out object ppReturnedDeviceInterface);
    ...
    public interface IDirect3D9
    {
        ...
        [PreserveSig, SuppressUnmanagedCodeSecurity]
        int GetAdapterIdentifier(UInt32 Adapter, UInt32 Flags, UInt32 pIdentifier);

        [PreserveSig, SuppressUnmanagedCodeSecurity]
        UInt32 GetAdapterModeCount(UInt32 Adapter, D3DFORMAT Format);

        [PreserveSig, SuppressUnmanagedCodeSecurity]
        int EnumAdapterModes(UInt32 Adapter, D3DFORMAT Format, UInt32 Mode, [Out] out D3DDISPLAYMODE pMode);

        [PreserveSig, SuppressUnmanagedCodeSecurity]
        int GetAdapterDisplayMode(UInt16 Adapter, [Out]out D3DDISPLAYMODE pMode);
    ...
    public class Direct3D
    {
        ...
        [DllImport("d3d9.dll", EntryPoint = "Direct3DCreate9Ex", CallingConvention = CallingConvention.StdCall),
         SuppressUnmanagedCodeSecurity]
        public static extern int Direct3DCreate9Ex(UInt16 SDKVersion, [Out]out IDirect3D9Ex ex);

        public static bool FillD3DSurfaceFromBitmap(IDirect3DSurface9 surface, ref Bitmap bitmap)
        {
            try
            {
                D3DLOCKED_RECT pBitsDest;

                int width = bitmap.Width;
                int height = bitmap.Height;
                surface.LockRect(out pBitsDest, null, 0);

                Rectangle rect = new Rectangle(0, 0, width, height);
                BitmapData bitmapData = bitmap.LockBits(rect, ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
                IntPtr pSrcLine = bitmapData.Scan0;
                IntPtr[] pDestLine = new IntPtr[1];
                pDestLine[0] = pBitsDest.pBits;

                int dataSize = width * height * 4;
                byte[] data = new byte[dataSize];
                Marshal.Copy(pSrcLine, data, 0, dataSize);
                Marshal.Copy(data, 0, pBitsDest.pBits, dataSize);
    ...
    [CLSCompliant(true)]
    public static class DXConst
    {
        public const UInt16 D3DADAPTER_DEFAULT = 0;
    }
